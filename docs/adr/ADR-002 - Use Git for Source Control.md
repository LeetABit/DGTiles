# ADR-002: Use Git for Source Control
...
The project requires a version control system to manage source code
effectively. The chosen system should support scalability for future needs
while being efficient and reliable for individual use.
...
5. **Ecosystem and Tooling**:
   Git has an extensive ecosystem with integrations for hosting platforms
   (e.g., GitHub, GitLab, Bitbucket) and development tools, which can simplify
...
GitHub or GitLab), which could introduce minor additional steps for repository
management.
