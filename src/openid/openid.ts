...
import openidConfiguration from './openid.config.json';
...
export function getAuthorizationUrl(options: IdentityProviderOptions, nonce: string): URL {
    const endpointUrl = joinUrl(
        options.baseUrl,
        [
            `B2C_1A_${options.identityProviderName.toUpperCase()}`,
            'oauth2/v2.0/authorize',
        ],
    );
    const currentLocationWithoutHash = `${window.location.protocol}//${window.location.hostname
    ...
    const queryParams = {
        ...
        nonce,
        redirect_uri: joinUrl(new URL(currentLocationWithoutHash), ['signed-in', options.identityProviderName])
            .toString(),
        scope: 'openid',
...
export async function verifyIdToken(options: IdentityProviderOptions, nonce: string) : Promise<Optional<string>> {
    ...
    if (idToken !== undefined) {
        const openidConfigurationUrl = joinUrl(
            options.baseUrl,
            [`B2C_1A_${options.identityProviderName.toUpperCase()}`,
